    ...
    <div className='w-full text-text-100 h-full'>
       ...
       <div>
        ...
        <div className='flex w-full gap-6'>
          ...
          <div className='flex text-text-200 text-[12px] items-center flex-1 bg-[#231b2e] p-2 rounded-md cursor-pointer hover:bg-main-100 overflow-hidden'
               onMouseEnter={()=>handleHover(0)}
               onMouseLeave={()=>handleLeave(0)}
          >
            <div className='relative group overflow-hidden rounded-full'>
              <img ref={imageRef0} src={searchdata?.artists[0].thumbnailM} alt='avatar' className='h-[85px] w-[85px] rounded-full overflow-hidden'/>
              <div className={`absolute top-0 left-0 right-0 bottom-0 items-center justify-center bg-[rgba(0,0,0,0.3)] rounded-full ${isHover[0] ? 'flex' :'hidden'} text-text-100`}><CiShuffle size={24} /></div>
            </div>
            <div className='flex flex-col text-left pl-3'>
              <span>Nghệ sĩ</span>
              <span className='text-text-100 font-semibold text-[14px]'>{searchdata?.artists[0].name}</span>
              <span>{Math.floor(searchdata?.artists[0].totalFollow/1000)}K quan tâm</span>
            </div>
          </div>
          <div className='flex text-text-200 text-[12px] items-center flex-1 bg-[#231b2e] p-2 rounded-md cursor-pointer hover:bg-main-100 overflow-hidden'
          ...
          >
            <div className='relative group overflow-hidden rounded-full'>
              <img ref={imageRef1} src={searchdata?.artists[1].thumbnailM} alt='avatar' className='h-[85px] w-[85px] rounded-full overflow-hidden'/>
              <div className={`absolute top-0 left-0 right-0 bottom-0 items-center justify-center bg-[rgba(0,0,0,0.3)] rounded-full ${isHover[1] ? 'flex' :'hidden'} text-text-100`}><CiShuffle size={24} /></div>
            </div>
            <div className='flex flex-col text-left pl-3'>
              <span>Nghệ sĩ</span>
              <span className='text-text-100 font-semibold text-[14px]'>{searchdata?.artists[1].name}</span>
              <span>{Math.floor(searchdata?.artists[1].totalFollow/1000)}K quan tâm</span>
            </div>
          </div>
          <div className='flex text-text-200 text-[12px] items-center flex-1 bg-[#231b2e] p-2 rounded-md cursor-pointer hover:bg-main-100 overflow-hidden'
          ...
          >
            <div className='relative group overflow-hidden rounded-full'>
              <img ref={ imageRef2} src={searchdata?.artists[2].thumbnailM} alt='avatar' className='h-[85px] w-[85px] rounded-full overflow-hidden'/>
              <div className={`absolute top-0 left-0 right-0 bottom-0 items-center justify-center bg-[rgba(0,0,0,0.3)] rounded-full ${isHover[2] ? 'flex' :'hidden'} text-text-100`}><CiShuffle size={24} /></div>
            </div>
            <div className='flex flex-col text-left pl-3'>
              <span>Nghệ sĩ</span>
              <span className='text-text-100 font-semibold text-[14px]'>{searchdata?.artists[2].name}</span>
              <span>{Math.floor(searchdata?.artists[2].totalFollow/1000)}K quan tâm</span>
            </div>
          </div>
       ...
       <div className='flex items-center'>
        <div className='overflow-hidden rounded-md'>
          <img src={searchdata.top.thumbnail} className='w-[50px] h-[50px] rounded-md cursor-pointer hover:animate-scale-up-image'/>
        </div>
        <div className='flex flex-col text-left pl-4'>
          <span className='text-[14px] text-text-200'>PLAYLIST NỔI BẬT</span>
          <span className='font-bold cursor-pointer hover:text-main-500'>{searchdata.top.name}</span>
        </div>
       </div>
       ...
       <div className='flex w-full gap-6 mt-5'>
        {searchdata?.playlists?.map((playlist, index)=>(
          <SectionItem key={playlist.encodeId} thumbnailM={playlist.thumbnailM} title={playlist.title} sortDescription={ playlist.sortDescription} artistsNames={  playlist.artistsNames} link={playlist.link.split('.')[0]}/>
        )
       ...
       <div className='w-full h-auto flex gap-3'>
           <div className='w-1/2'>
           {searchdata && searchdata.songs.slice(0, 3).map((item, index)=>(<SongItem key={item.encodeId} thumbnail={item.thumbnailM} title={item.title} artists={item.artistsNames} timeRelease={item.releaseDate} songId={item.encodeId} playlist={searchdata.songs}/>))}
          
           </div>
           <div className='w-1/2'>
           {searchdata && searchdata.songs.slice(4, 7).map((item, index)=>(<SongItem key={item.encodeId} thumbnail={item.thumbnailM} title={item.title} artists={item.artistsNames} timeRelease={item.releaseDate} songId={item.encodeId} playlist={searchdata.songs}/>))}
          
           </div>
        
       </div>
       </div>
       <Section sectionType={searchdata} artists={false} sortDescription={true} title={'Playlist/Album'}/>
       </div>
       </Scrollbars>
    </div>
  )
